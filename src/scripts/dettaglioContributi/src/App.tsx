...
import getDettaglioContributi, { DettaglioContributo, Linea } from "./services/getDettaglioContributi"
...
const mapToDataChart = (dettaglioContributi: DettaglioContributo[], historicalData: HistoricalData | undefined, from: Date | undefined): DataChart => {
              ...
              .reduce<number>((acc, linea) => {
                if (!historicalData || historicalData[linea]?.length === 0) {
                  ...
                  if (historicalDataFiltered?.length === 0) {
                    const totImporto: number = (dettaglioContributiFiltered.length > 0 ? dettaglioContributiFiltered[dettaglioContributiFiltered.length - 1][linea]?.importoNettoCumulato : 0) || 0
                    return totImporto
                  } else {
